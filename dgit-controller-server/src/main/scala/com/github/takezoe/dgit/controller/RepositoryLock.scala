      ...
      try {
        db.transaction {
          val lock = db.selectFirst(
            sql"SELECT COMMENT, LOCK_TIME AS COUNT FROM REPOSITORY_LOCK WHERE REPOSITORY_NAME = $repositoryName"
          ...
          db.update(sql"INSERT INTO REPOSITORY_LOCK (REPOSITORY_NAME, COMMENT, LOCK_TIME) VALUES ($repositoryName, $comment, $timestamp)")
        }
        ...
        db.transaction {
          db.update(sql"DELETE FROM REPOSITORY_LOCK WHERE REPOSITORY_NAME = $repositoryName")
        }
      }
    } catch {
